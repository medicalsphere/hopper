"""
Perplexity adapter — uses the OpenAI SDK against Perplexity's Responses-compatible endpoint.

Install: pip install openai
Docs: https://docs.perplexity.ai/docs/agent-api/openai-compatibility

Perplexity exposes a /v1/responses endpoint that is compatible with the OpenAI
Responses API. The OpenAI SDK is pointed at the Perplexity base URL; everything
else (input array, instructions, max_output_tokens, streaming) is identical to
the OpenAI adapter.
"""
...
def _build_input(request: CanonicalRequest) -> list[dict]:
    """Build the Responses API `input` array. System prompt is passed via `instructions`."""
    messages: list[dict] = []
