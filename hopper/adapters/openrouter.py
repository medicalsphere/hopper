"""
OpenRouter adapter — uses the OpenAI SDK against OpenRouter's OpenAI-compatible
Chat Completions endpoint.

Install: pip install openai
Docs: https://openrouter.ai/docs/api-reference

OpenRouter mirrors the OpenAI chat completions request/response shape, so
build_openai_messages() and choices[0].message/usage parsing apply directly.

- max_tokens is accepted as-is (no renaming needed).
- thinking is moved into extra_body since it is not part of the
...
def _translate_params(params: dict) -> tuple[dict, dict]:
    ...
    OpenAI SDK's chat.completions.create() signature.
    max_tokens is passed through unchanged (OpenRouter accepts it natively).
    """
    api_params: dict = {}
    extra_body: dict = {}
        ...
        try:
            ...
            async for chunk in response:
                ...
                if choice.delta.content is not None:
                    yield StreamChunk(delta=choice.delta.content, finish_reason=None)
